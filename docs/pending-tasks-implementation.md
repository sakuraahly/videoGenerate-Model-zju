...
**现状（九审定稿·实测定案——更正原"无 Ref2VA 模板/第一步须探测"表述）**：本地镜像共 7 份模板（`workflows/remote_workflows/`），**全部是 UI 格式**（按内容判定：nodes/links/widgets_values；3 份名为 `api_*` 的属 Comfy 云通道模板，不是 API 格式；本地可执行的是 4 份 `video_*`）。其中 `video_minimax_h3_r2v.json` **就是 Ref2VA（ref2va）模板**：已含 `MiniMaxH3ReferenceToVideo`（id 136）+ 槽位 `ref_images.ref_image_0..7`（IMAGE，上限 9）/`ref_videos.ref_video_0`（IMAGE 帧序列，上限 3）/`ref_video_audios.ref_video_audio_0`（AUDIO，上限 3，与同号视频配对）/`ref_audios.ref_audio_0`（AUDIO，上限 3），仅 `ref_video*`/`ref_audio*` 未接线（link=null）；模板内 8 张 LoadImage 仅 2 张已接（ref_image_0/1），其余为死链（转 API 时被 `prune_dead_output_nodes` 清理）。**spark 已实证**：核心节点 `LoadVideo`（comfy_extras/nodes_video.py：仅 1 个 file COMBO，输出 VIDEO）与 `LoadAudio`（nodes_audio.py：仅 1 个 audio COMBO，输出 AUDIO）存在；同事模板 `utility-gan_upscaler.json` 给出拆帧链实证：`LoadVideo → GetVideoComponents`（VIDEO→images IMAGE + audio AUDIO + fps/bit_depth/color_space）。**关键格式约束（实测 object_info）**：`ref_videos.ref_video_N` 输入类型是 **IMAGE（24fps 参考视频帧，2-15s）**——LoadVideo 不能直连，必须经 GetVideoComponents 拆帧/拆声（images→ref_videos、audio→ref_video_audios 同号配对）。slot 口径与 7a 目标一致（videos ≤3 / audios ≤3 / images ≤9）；capabilities.json 现有 `video_r2v` 条目 slots.images=8、features.reference_videos=false。
**分三步实现（每步可独立验收）**：
- **7a 探测登记（九审修订：探测已提前完成，实施期仅复核版本）**：复核 `MiniMaxH3ReferenceToVideo`/`LoadVideo`/`LoadAudio`/`GetVideoComponents` 的 object_info（`/object_info/<type>` **只读、不需队列空闲**——见 §0 新行）与本地镜像完整性 → 结果写入 capabilities.json 新 stage `video_ref2v`（slots=videos up to 3 / audios up to 3 / images up to 9——与实测定案口径一致）与 code-fact-registry。**登记后必须补全（九审中项）**：`workflow_registry.add_local`（workflow_registry.py:177-200）写入的是**空 slots + features 全 false** 的默认值（:190-196），且其返回消息即写明"请补全 slots/inject_spec 并 validate"——走默认路径登记出的 `video_ref2v` 是 slots 全空 + reference_videos:False + audio:False，与目标相反；**方案＝扩展 `add_local` 接受 slots/features 参数（推荐），或登记后 patch + `validate_all`**（二选一，禁止只登记不补全）。目标条目：slots={images:[{role:reference,count:9}], videos:[{role:reference,count:3}], audios:[{role:reference,count:3}]}、features={reference_videos:true, audio:true, negative_support:true}、template=`workflows/remote_workflows/video_minimax_h3_r2v.json`、prompt_inject/inject_spec 按 `video_r2v` 现条目同构（inject_spec 已被现有 engine 使用）。**注意**：`template_health` 的槽位检查目前只数 LoadImage（workflow_registry.py:127-130），需扩展为按 slots 规格数 LoadImage/LoadVideo/LoadAudio。若复核失败（节点/槽位缺失或版本不符）→ **标记『不可行（环境缺能力）』并如实归档**，不做任何臆造接线（取舍保留，见下）；
- **7b 引擎接线（九审修订：补两个必需落点 + 修正参数签名）**：
  - **落点 1（新增）：`uiapi.py` 文件选择器类节点转换分支**。依据（实测）：`LoadVideo`/`LoadAudio` 的 object_info 只声明 **1 个必需 COMBO**（`file`/`audio`，cfg 含 `video_upload`/`audio_upload` 标记），但 UI 节点 `widgets_values` 记 **2 个值**（文件名 + upload 展示值 'image'；实证 `utility-gan_upscaler.json` node 9）→ 走通用路径（uiapi.py:277-306）消费 1 值后触发 :302-305 的"widget 值无法按定义分配"→ `UiUnsupported`。**分支逻辑＝与 LoadImage 特例（uiapi.py:273-276）并列：文件选择器类节点只取首值写入对应输入名（LoadVideo→`file`、LoadAudio→`audio`）并 `widgets.clear()`**。无此分支则 7b 注入必失败（转换即抛错），这是 7b 完成度的必要判据（见验证一级）。
  - **落点 2：`bind_refs_to_template` 接线链**（新建函数，`runs/h3/refimage.py`，与 `bind_images_to_template`（:175）同文件并列）：签名 **`bind_refs_to_template(stage, images, videos, audios, template)`——`template` 设为必填**（九审中项：`bind_images_to_template` 的 template 默认 None→回退共享模板并**原地写回**（refimage.py:184/201）正是 book-11 模板污染事故来源；新函数无历史兼容负担，从签名上消除该模式，勿沿用"调用方自觉传副本"的纪律）；操作对象=任务目录副本（沿用 h3_submit.py:484-487 的 copy2+绑定模式），**并发安全=参照 h3_batch 的 batch_lock 模式**。绑定内容：images→既有 LoadImage 槽位（复用现有逻辑）；videos（N=1..3）→新建 `LoadVideo`+`GetVideoComponents` 节点并接线：GVC.images→`ref_videos.ref_video_N`、GVC.audio→`ref_video_audios.ref_video_audio_N`；audios（N=1..3）→新建 `LoadAudio`→`ref_audios.ref_audio_N`。
  - **上传顺序（九审补）**：videos/audios 先上传到 spark ComfyUI **input/ 根目录**（LoadVideo/LoadAudio 的 COMBO options=input 根目录文件列表；现有 `--image` 链即"先上传后转换"，机制同源）→ 再写 `widgets_values[0]`=远端文件名 → 最后转换提交。`h3_submit` 增 `--videos/--audios`（append，同 `--image` 语义）；上传复用 `ComfyClient.upload_image` 或新增通用 `upload_file`；`capabilities` 注册 params（resolutions/seconds 沿用 r2v 口径）。
- **7c 工具/提示词**：`tools.py call_comfyui` 增 `videos/audios`（同 `images` 语义：路径或 spark input 文件名；上限 3/3）；SYSTEM_MESSAGE 加 `<Video N>/<Audio N>` 提示词规范（**N 与槽位序号一一对应**：`<Video N>`→`ref_videos.ref_video_N`（其配乐随 `ref_video_audios` 同号配对、不另占 tag）、`<Audio N>`→`ref_audios.ref_audio_N`；参考素材按连接顺序引用）与使用边界（视频=动作/运动/风格参考；音频=氛围参考；不一致会导致编辑层重音轨，见下）；
...
**验证（九审两级判据——原"dry-run 断言图注入"离线不可达成，作废并替换）**：
- **一级（转换/注入判据，需在线 client；`/object_info` 只读、不需队列空闲，见 §0）**：绑定后 `convert_ui_file` 产出 API dict 断言——① 存在 class_type `LoadVideo`/`GetVideoComponents`/`LoadAudio`；② `LoadVideo.inputs` 只含 `{"file": <name>}`、`LoadAudio.inputs` 只含 `{"audio": <name>}`（无 `image` 展示值——证明文件选择器分支 `widgets.clear()` 生效、无展示值混入）；③ `MiniMaxH3ReferenceToVideo.inputs` 含 `ref_videos.ref_video_0=[gvc,0]`、`ref_video_audios.ref_video_audio_0=[gvc,1]`、`ref_audios.ref_audio_0=[la,0]`（只含预期 key）。该层可单测打桩（mock `fetch_object_info`/client，无 ffmpeg、不占生成队列）。
...
- **dry-run 边界**：dry-run 只能断言 **UI 层图注入**（绑定副本上的节点/槽位/link 计数）；API 层断言必须有在线 client——`stage.load_api_or_ui_template` 对 UI 模板无 client 抛 ParamError（stage.py:322-326），故"dry-run 断言图注入"作废（原表述来自对"模板可离线转 API"的误判）。
- **若 7a 复核失败（无节点/无模板）→ S7 标记为『不可行（环境缺能力）』并如实归档**，不做任何臆造接线。工作量：**大（7a 小 / 7b 中-大 / 7c 小）**——7b 上调（九审：新增 uiapi.py 文件选择器转换分支 + GetVideoComponents 拆帧/拆声链 + 上传顺序 + 登记后补全 + 两级验证判据，5 个落点）。
