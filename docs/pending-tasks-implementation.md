...
| 1080p-级输出 | 4x 模型 608→2432×1408（测试帧 1216→4864×2816），按目标 lanczos 降采样 | 可行但标注修正（十六审查实：**模型本体不施加 768p**——MiniMaxH3ReferenceToVideo width/height max=16384/step=32、ResolutionSelector megapixels max=16.0、官方标称 up to 2K；**768p=项目侧硬编码 6 处**：workflow.py:18-24 RESOLUTION_PRESETS（顶 768p）/tools.py:260/792/capabilities.json 四 workflow params.resolutions/workflow_registry.py:193 默认模板/h3_text2img.py:40 重复表；**原生 1080p/2K 探测无需改代码**——params.py:200-204 已有显式 width+height 逃生口（%8 校验、64-4096）+ h3_submit.py:285-286 `--width/--height`（**CLI-only：tools.py 全文 0 命中 width/height，agent 路径够不着**）；**口径冲突**：逃生口 %8 vs 节点 step=32——正确探测值=**1920×1088**（均 %32；朴素 1920×1080 会通过项目校验但违反节点约束）；项目上限 4096 < 节点 16384；探测提交=1920×1088 + `--lora none`（20 步）一次定三事（墙钟/显存、ref2v_4step/8step 在 768p+ 是否可用、服务端是否强制 step=32）——**待用户授权队列空闲窗口**；若原生成立：P1b 分辨率立论消失、§13"超分合成非原生"改"原生可达 1080p 级"、S2-v2 优先级重排；**另记录**（十六审）：ref_images tooltip 原文"downscaled to 2048 short edge if larger, never upscaled"→参考图修复/增强产物有效分辨率**封顶 2048 短边**（§13 Inpaint 行适用）） |
