...
- **7b 引擎接线（十审定稿：主案=API 层注入，循 apply_lora 先例——可省九审落点 1 全部与落点 2 大部）**：
  ...
  - **落点 3（上传顺序，九审保留）**：videos/audios 先上传到 spark ComfyUI **input/ 根目录**（LoadVideo/LoadAudio 的 COMBO options=input 根目录文件列表；现有 `--image` 链即“先上传后转换”，机制同源）→ 后注入 `file`/`audio`=远端名；`h3_submit` 增 `--videos/--audios`（append）；上传复用 `ComfyClient.upload_image` 或新增通用 `upload_file`；`capabilities` 注册 params（沿用 r2v 口径）。
  - **备选（设计 A，不实施·记录供将来翻案）**：若要模板可往返调试（模板副本里可见参考节点），须做：① uiapi.py 文件选择器分支（LoadVideo/LoadAudio 首值+widgets.clear()，与 LoadImage 特例 uiapi.py:273-276 并列；判据=九审实证的 2 值 widgets→:302-305 UiUnsupported）；② UI 行合成——**先例= grow_slots（refimage.py:497-526：`ref_image_N` 行追加+`_clone_loadimage` 克隆占位）**，泛化到 `ref_videos/ref_video_audios/ref_audios`（AUTOGROW 行含 name/type/link/shape/widget；LoadVideo/GetVideoComponents/LoadAudio 节点克隆）；③ `_wire_slot`（refimage.py:541-556）参数化——:551 链接类型硬编码 “IMAGE”→按槽位取值（AUDIO）、:554 源输出匹配硬编码（“IMAGE”/“”）与输出槽 0→按需取槽（GVC audio=槽 1）、:547 假定目标行已存在（A 下先合成行）；④ 簿记同步（last_node_id/last_link_id）。工作量≈大上沿（3 视频槽）。
- **7c 工具/提示词（十审修正：tag=1-based、槽位=0-based，差 1 修正）**：`tools.py call_comfyui` 增 `videos/audios`（同 `images` 语义；上限 3/3）；SYSTEM_MESSAGE 加 `<Video N>/<Audio N>` 提示词规范——**`<Video N>`↔`ref_videos.ref_video_(N-1)`、`<Audio N>`↔`ref_audios.ref_audio_(N-1)`**（官方 tag 从 1 起按连接顺序、槽位键从 0 起；上一版“ref_videos.ref_video_N”差 1 已更正）；`<Video N>` 的配乐随 `ref_video_audios` 同号（N-1）配对、不另占 tag；使用边界（视频=动作/运动/风格参考；音频=氛围参考；不一致会导致编辑层重音轨，见下）。
