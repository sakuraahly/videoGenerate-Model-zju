...
| 1080p-级输出 | 4x 模型 608→2432×1408（测试帧 1216→4864×2816），按目标 lanczos 降采样 | 可行但标注修正（十四审：官方文档标称模型原生 up to 2K——当前链路上限 768p 的来源待查【加速 LoRA 训练/受限 vs 模型本体】；**原生 1080p/2K 待探测**：不带 LoRA 提交一次 1080p/2K 看节点是否接受/画质；探测前不做定论；若原生成立则"超分合成非原生"标注改为"原生可达 1080p 级别"；P6 需视觉抽检） |
