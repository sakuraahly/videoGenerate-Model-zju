...
> 现状时间点：**八轮外部审核全部闭环**；涉及代码均已修复+单测+真机验收；仓库双端干净。
>
> 一句话现状：规划书 `docs/pending-tasks-implementation.md`（S1–S13 + P2–P6）经 **8 轮审核**定稿，
...
4. Ref2VA 节点/模板探测（7a：`/object_info` 枚举 `MiniMaxH3ReferenceToVideo` inputs + spark 同事模板目录）——S7 动工第一步；
