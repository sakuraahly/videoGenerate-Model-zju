...
| 现场事故修复（2026-09-06） | 上传摘要回显文件名清单+dup 不再重复镜像+>3 张缩略图并行+demo.load 清空 up_btn（防 Gradio InvalidPathError 中断）；分镜多段提交用 batch_submit（call_comfyui 每轮 1 次频控）；i2v 首帧偏离=模型弱锚定（非绑定 bug，需强保真用 flf2v）——详见 reference-2026-09-04.md §9 故障字典 |
