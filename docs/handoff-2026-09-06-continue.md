...
**剩余=☆真机验证（用户已确认意图=参考图贯穿全片→走 r2v）**：r2v 提交 3 段×每段抽 3 帧（首/中/尾）目检：人物身份/场景空间/道具外观全程一致、不再首尾帧化——需用户配合（队列空闲窗口+r2v 3 段提交+抽帧目检判据）。**勘误（2026-09-06 核查）**：原首验 3 段实为 flf2v（首尾帧转场，设计语义；非 r2v 缺陷），已从 P1.5 验证基准剔除；P1.5 修复针对 r2v 通道真实契约缺失（12 份历史 r2v 提交全部无 tag+match）。遗留断点 5f820fe9 已核查=3 段全部成功（产物 00112/00113/00114 已取回 win outputs video_33/34/35）并清断点。（首/中/尾）目检（人物身份/场景空间/道具外观全程一致、不再首尾帧化）——需用户配合（队列空闲窗口+3 段 r2v 提交+抽帧目检判据）。
