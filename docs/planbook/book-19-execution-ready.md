...
**夜间窗口首轮执行（2026-09-07 晚，BMJ）**：①1080p 原生探测✅PASS（night_runner --auto 自动提交+验证+标记；`MiniMax_H3_00174_.mp4`=1920×1088 h264+aac 5.167s/124帧；outputs/video_53.mp4 同步）②Wav2Lip 口型冒烟✅（权重=HF camenduru（Windows hf-mirror 下载→scp 436MB+90MB）；代码=codeload；依赖=tts-venv+清华 pip（opencv-headless/librosa/tqdm）；输入=00171 人脸+cosy 台词 6.04s；产物=outputs/w2l_smoke_talk.mp4（h264+aac）+前后帧证据图；排障两连：librosa≥1.0 mel 必须关键字参数、opencv5-arm64 无视频写入后端→PNG 序列+系统 ffmpeg）③RIFE⛔渠道阻塞复核属实（MS 无官方/精简版与 ComfyUI 不兼容；GH 两库均无 release，README=Google Drive+colab；HF 仅 train_log 副本（IFNet_HDv3/flownet.pkl）；SVFI 1.x=VS 工程源码无权重——官方权重不可达→登记远期，夜窗移除）④4x 超分叠加（RealESRGAN 4x-UltraSharp on 00170_ 1216×672 12.25s——夜窗内执行，结果见 CURRENT-STATE §9）。
...
**⑦ 冒烟 PASS（2026-09-09 白天，人像模式）**：nt-echomimic-smoke 全链成功—— [--ref-image] 父亲.png(1600×2848) + line.wav(1.5s/66KB) → MTCNN 原图判据(5 脸/最大脸) → EchoMimic infer_audio2vid_acc.py(fp16/6 步/512²/24fps) → _withaudio.mp4(带音轨) → FINAL /tmp/echomimic_smoke.mp4(265KB) → DONE_ECHO_TALK；抽帧目检=白发老者正面像/口型合成中/无贴框。**排障补充(⑦)**：①echomimic_talk.py 人像模式入口 bug——[--video] 默认 '' + 无条件 is_file 拦截 [--ref-image] 路径(报"视频/音频不存在")→ 已修(视频/参考图至少其一,各自独立校验; dry-run 兼容)②select_face 与 EchoMimic 内 np.round 崩("float has no rint")= facenet_pytorch 2.6.0 在 numpy1.26 下返回 object dtype(2.5.0=float32 正常)→ 降回 2.5.0(requirements 本就 pin 2.5.0)+ select_face 显式 np.asarray(float) 防御③moviepy 2.x 无 moviepy.editor → 降回 1.0.3(editor API)④em_out 相对路径(output/...)在 EM_ROOT 下产生 → copy 失败 → 已按 EM_ROOT 解析⑤tts-venv 教训复核：装完必验 torch.cuda.is_available()。**下一步**：接 lipsync 链 [--talking-backend echomimic](替 W2L+GFPGAN 贴皮段) → 真机交付 video_N；视频构图模式(带背景回贴)另测。

## 15f. 魔搭创空间打包目标（2026-09-08 用户定案）

**目标**：本项目整体打包上传魔搭创空间（ModelScope Studio）。**调研已完成**（创空间=git 仓库+token 发布、免费 CPU 2vCPU/16G+休眠激活、付费 GPU 升配=PAI、app.py 核心；出网部署 ComfyUI 有社区尝试但受 GPU 规格限制）。适配不可整体迁移引擎（GB10+40GB 模型）——**分层**：创空间=展示+交互入口；spark=引擎层（公网 API 调度）。实施序=M1 静态展示版（免费 CPU：片墙+流程+演示表单）→M2 远程调度版（studio_gateway.py 鉴权 API + 创空间出网实测）→M3（免费 GPU→单点模型演示）。**文档写就**：`docs/guides/studio-porting.md`（事实表+设计+里程碑验收）；**skill 卡**：`skills/studio-packaging.md`。**M1 已实施（2026-09-08 用户确认后动工）**：`studio/` 目录=创空间项目根——`app.py`（Gradio 配置驱动：项目介绍+能力点+6 部样片墙(压缩≤220KB+封面)+五步流程+演示表单→演示结果卡+风格匹配样片）、`config.yaml`(sdk: gradio 4.44 + 展示元数据)、`requirements.txt`、`README.md`、`assets/`(12 文件≈1.2MB)。**证据**：本地 Gradio 6.26 启动→HTTP 200→`/config` 断言 53 组件/13 媒体组件/1 表单依赖。**验收 ✅ 通过（2026-09-08 深夜，用户确认'运行中/都有/很正常'）**：空间页运行中、Gradio 应用内嵌正常（片墙播放+演示表单）。M2 远程调度待出网实测后实施。
