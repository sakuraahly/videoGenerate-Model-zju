...
| 11 日志 | 全场景/无垃圾/不错失 | ⬜ 未开始 | 仅记录设计；待做 |
| 12 工具/工作流模块化 | 注册表驱动/便捷更换 | ⬜ 未开始 | 仅设计+capabilities 雏形；待做 |
...
1. **端到端黄金路径闭环（book-09 步骤 7）**：上传 2 图 → agent 用 `images` 参数绑定 → 提交（守卫应放行）→ 取片 → 断言各段/视频**内容与参考图对应、非旧资产**（防"别人的图"再现）。
