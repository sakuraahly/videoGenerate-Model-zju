...
**S14（2026-09-05 用户实测新增·✅ 已修复）：上传状态与预览强耦合**——用户反馈：文件管理器选择并点击上传后，直到预览图出现在界面，提示文字才开始变化（卡顿感知=选择→handler 传输间隙 + ingest 内缩略图同步阻塞）。修复：① `up_btn.select` 前置状态（选择即显示「已选择 N 个文件，正在上传…」，条件绑定防版本差异）；② `ingest_upload` 与缩略图**解耦**（不再内部生成缩略图，返回 (源,sha)）；③ `_upload` 分段状态：上传/归档中 → 逐张「正在生成预览 i/N…」→ 完成。真实链验证（gradio 缓存文件上传）：SSE 帧序列=正在上传→正在生成预览→✅，gallery 2 项。**遗留**：浏览器端 select 事件的实际显示需用户实测确认（API 链无法触发）。
...
## 6. 全池复核（2026-09-08，用户要求'池子观察能干的现在干'）

**复核结论：遗留池已清账**——早期各册标'未完成/待做'的条目绝大多数已被后续册落地（册头'计划(未实施)'多为创建期模板文案，未随实施刷新；本表为准）：

| 条目 | 原属 | 现状（2026-09-08 复核） |
|---|---|---|
| §3.4 六工具描述注册表派生 | book-12 | ✅ 已做（A 阶段⑤：`tools.py _derive_tool_enums/_apply_registry`，runs/agent/tools.py:985-1038） |
| §3.5 状态轮询成本 O(1) | book-07 | ✅ 已做（book-19 S8：`h3_batch cmd_status` 用 ComfyClient 决策树 queue_pids+history，无逐段子进程） |
| P-05 截断自动完成 | book-04 | ✅ 已做（book-19 §8 P0 受控续接：目标驱动+上限5+轮空熔断） |
| 消息级分批渲染（流式） | book-03/10 | ✅ 已做（book-13 C1：chunk 事件消息级分批 yield） |
| 黄金路径闭环 | book-09/10 | ✅ 已做（tests/golden_path.py 引擎级闭环 2026-09-04） |
| idea2prompts --segments N | book-06 | ✅ 已做（book-19 S4：--segments-json 已实施） |
| agent 成品无字幕（T2b v1） | book-16 #13 | ✅ 已做（T2b v2：字幕烧录/keep 语义 subtitle_source=text） |
| 服务守护（agent/sglang 无守护❌） | book-15 | ✅ 已做（2026-09-07 起 tmux supervisor 治理，CURRENT-STATE §2） |
| dev.py logs（book-11 日志治理） | book-11 | ✅ 已做（dev.py cmd_logs：view/check/clean/agent-log） |
| `_SCRIPT_TIMEOUT`(120s) 与 h3_batch(600s) 错配 | book-07 | ✅ 已闭合（2026-09-08：120→600 + batch 默认 100→600 + 注册表） |
| §3.2 图片解析收敛（assets.py 共享） | book-13 S11 | 维持原判定：价值/风险低，登记不做（触碰引擎三处调用） |
| nap()/supervisor 语义冲突 | book-13 #16 | 维持观察：涉及共享机 supervisor 服务侧逻辑（非本仓库面），需服务侧授权评估；判定不归本仓实施 |
| '最近会话共享区(显式)' | book-05 待定 | 低优先：已有 grant_refs→shared-<cid> 精授权替代；用户频繁跨会话时再评估 |
| book-08 风格回归（真实对话样本调优） | book-08 | 待用户提供一次完整对话任务样本（输入型，不阻塞） |
| S5 销毁性自愈演练 / nap-wake 专项逐测 | book-10/19 | 待用户授权（涉及休眠/销毁性操作） |
| 修复乱码/局部重绘 Inpaint 模板 | book-13 #15② | 远期候选：需新模板工程化+注册；未启动（登记） |

**结论**：无'现在就可干但没干'的遗留——剩余项全部=已闭环、策略性不做、或需用户输入/授权。后续池增减以本表+book-19 §3 为准。
