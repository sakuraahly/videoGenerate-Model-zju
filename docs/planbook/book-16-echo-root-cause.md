...
> 状态：**✅ 归档闭环（2026-09-05）**——根治完成（自管循环/tools= 格式/解析/回填/频控/去重）；用户四问处置（§6）；语音链移交 book-14（T2b v2 完成）；台账 #1-#14 全部关闭（#11b/#12-14 见台账）；验收口径 §6.5 与测试纪律 §6.6 生效并挂入 dev-workflow §8/§12。
...
| 11 | TTS 钩子挑错文件（glob-mtime 命中 video_22 而非本次 video_23）+ 截断源再入 → **我对“video_23 含中文旁白”误报** | **已修（book-14 T2b v2 #1/#2）**：钩子只取本次 `_local_out[0]`；时长守卫 <80% 拒绝；真实链 `TTS_OUT: outputs/video_27.mp4 … srt=yes`（本任务文件+完整时长） | 完成 | - |
| 11b | 验收口径强化（§6.5）：音轨=合成语音指纹/证据链一致性 | **已生效**：PROBE+TTS_OUT 回执+帧目检（video_27/28）+ book-14 §验收 | 完成 | - |
| 12 | 模型自动续接**重复提交同任务**（10:40 三连/10:59 双发） | **已修（双层）**：① 会话级指纹去重（归一化 300 字符，30 分钟复用）；② 引擎断点守卫（last_job 未完成→拦截+征询）；真实链 round3 实证 = 引擎守卫拦截并复用原任务（模型措辞变化绕过去重层时仍有守卫） | 完成 | - |
| 13 | agent 成品**无字幕** | **已修（T2b v2 #3）**：台词→SRT→烧录（video_27/28 字幕帧目检“再见了，故乡。”清晰） | 完成 | - |
| 14 | **测试纪律**（多驱动/force_new 重放造成三连/双发与 ≈16 GPU 分钟占用） | **已修**：§6.6 纪律（一意图一驱动/队列只读等待/不擅自 force_new/承诺前验证文件本身）已写入并与 dev-workflow §8/§12 联动；本会话后续测试均单驱动单会话（book-15 演练/真实链验证均一意图一驱动） | 完成 | - |
