...
| `config/prompt_blueprints.json` | 增加「图像属性 token 保留清单」（依 `docs/guides/prompt-taxonomy.md` 10 正 + 9 负）与「逐段转场提示词」指导 | 供 idea2prompts 生成属性词+逐段描述 |
...
## 9. 待用户输入 / 待定项
- ✅ 图像属性 token 清单已定：见 `docs/guides/prompt-taxonomy.md`（10 正向 + 9 负向大类）；模板只保留基础质量/属性词，其余由 agent 注入。
