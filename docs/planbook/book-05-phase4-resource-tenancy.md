...
- ✅ **体验补丁（2026-09-04 用户实测："这两张图片"但新会话无素材）**：空会话时会附带"最近其他会话上传"线索（**仅线索、不越权**），并引导"请用户明确授权并指明；授权后可用 session=all"；SYSTEM_MESSAGE 增加同类引导（用户引用"这些/那两张图"时→说明+列线索+请确认授权）。实现：`refimage list --hint-recent N` + `tools.list_references` 空会话自动附带。spark 实测：列出最近上传（会话×、图名、时间）✅。
