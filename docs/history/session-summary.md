...
- **S7 二级真机 PASS（14:2x-14:41）**：queue_watch 复检 3/3 → --submit-only 入队（排队 1 用户任务后先跑）→ h3_submit --resume 轮询至 success：prompt_id=df684e84-d48e-48fe-86dc-fa5e2a197200；产物 spark outputs/video_42.mp4→win outputs/video_46.mp4（PROBE 608×352/124f/5.167s+AAC 32kHz/2ch/5.167s）；抽帧 0.5s/4.5s 目检：场景锁定+推近运镜（<Video 1> 驱动采纳）；<Audio 1> 采纳=听测待用户/ASR（如实标注，登记 P 链④）。
- **P 链④ ASR 客观验收冒烟（2026-09-06 落地）**：独立 asr-venv + funasr-onnx + `iic/SenseVoiceSmall-onnx`（241MB；bpe 需补拷；numpy 未污染 ai/venv 已恢复）；新工具 `runs/h3/asr_check.py`（ffmpeg→SenseVoiceSmall(quantize)→ASR_TEXT/VERDICT）。对 S7 产物 video_46 音轨首测：ASR 文本=英文乱语（`you are and a seminal brings the blood on that is to serated it`）——用户反馈④「胡言乱语」获客观证据；参考媒体音频=采纳但不复刻（语义边界已登记）；可辨析语音=tts_text 链（已有）。
- **P 链① 人声本地化 F5-TTS 冒烟 PASS（2026-09-06）**：tts-venv + 魔搭 F5TTS_v1_Base(1.35G)+vocos(54M,经 hf-mirror 下载——HF 直连不通;bigvgan 需 git submodule 弃)+ 新工具 `runs/h3/tts_local_check.py`;合成 24kHz/4.87s/CPU 53s;ASR 回环还原原句(可辨析可见);接入生产(替代 edge-tts 钩子)=后续里程碑(速度/参考音源/GPU 评估),edge-tts 过渡保留。
...
- **☆真机**：10:40 会话 agent 提交 720p/15s=e009acf1 → 立即 cancel_task → “任务已取消成功（归属校验通过；排队中移除；断点已清理）”——工具链完整闭环；后续查询将收到已取消（mark 遮蔽生效路径由单测覆盖）。
...
- **关键发现（agent 全链真机暴露 P1.5 缺口）**：第1轮任务 workflow_api.json wired_slots=2（模板默认仅 0/1 槽接线，2/3 为禁用占位）——bind_images_to_template 只写 widget 值不接线 → N>2 参考图时第 3+ 张静默不达模型（tag 契约全过+提交成功+参考缺失=最隐蔽缺陷）；产物印证：父亲=黑发络腮胡≠参考白发老人（形象漂移）。
- **修复**：refimage.py bind_images_to_template 补丁=绑定前自动启用前 N 个 LoadImage+接线 ref_image_0..N-1（幂等/行不足补行/失败不阻断+仅警告）；新增 tests/test_bind_slots.py 3 例；165 基线+3 新例绿。
- **修复验证（引擎直调同一链）**：34343657 提交 wired_slots=4/tags{1..4}/max PASS；产物 video_42（源 00121，608×352/8s）抽帧目检：父亲白发✅（对比修复前黑发络腮胡）、眼镜黑框✅、场景全程一致✅——缺口修复闭环。
- **可复用程序（用户要求重复操作脚本化）**：shell/agent_drive.py（新会话→上传→发消息→取回复，参数化）；spark 侧例行（绑定/提交/轮询/抽帧/证据）均已脚本化。
### 20.39 用户验收反馈 + agent 全链指令注入（2026-09-06）
- **用户验收 3 段 r2v**（视频效果反馈·已登记 book-19 §11，**当前不重跑**）：字体模糊/音乐不自然/脚步声奇怪且停止后仍存在/语音不清晰疑似胡言乱语/第三段桌上钟变眼镜穿帮——全部登记为升级工作流（S2 超分/音乐音效链/ASR 客观验收/参考语义）联动项。
- **流程指示（用户）**：① 向 qwen（7860 调度器）传输指令要**模仿用户行为**（例如通过工具把图片传进会话资源池，再把需求以用户口吻发出），让它自己调用工具跑流程——不得生硬注入；② 之后遇到的**反复出现重复操作一律撰写计算机程序执行**（本会话已按此执行：spark 侧验证/绑定/提交/轮询/抽帧全脚本化；示例经验：手修太脆→直接重写干净脚本）。
- **agent 全链驱动计划**：上传 4 参考图（浏览器→gradio→uploads 会话池）→ 用户口吻消息（8s r2v 一段，参考贯穿）→ 观察 agent 自调工具（list_references/call_comfyui）→ 产物抽帧+检查其提示词 tag 契约——P1.5 agent 侧真机验证。
...
- **行为闭环**：单段提交成功后自动续接查询/取片汇报；失败修正重试不再“自熄”（续接消息现在携带工具结果，判定不再依赖关键词）；批量成功后监控接管不空转；超过 5 次停+提示用户。
- **验证**：COMPILE_OK+165 基线；☆真机待用户重试（多段分镜）确认；scheduler.py CLI 路径同步=登记低优先。
...
- **根因 B（重试被频控拦死）**：`_tool_count` 在**每次执行前**计数（含失败）→ batch_submit 第一次失败后，修正参数的重试被“[频控] 本轮已执行 1 次”拦截 → 模型停在半途。**已修**：失败调用不占用频控名额（`max(0, count-1)` 回滚；成功/同指纹去重/会话限流/熔断语义不变）——失败重试合法，杜绝“多段批量提交中断”。
- **伴生**：batch_submit images 中文描述报错信息本身正确（“先用 list 查看 id/文件名”；refimage.py:380-399）——模型应先用 list（其行为已正确）；SYSTEM 铁律待实施。
- **验证**：COMPILE_OK + 165 绿；spark 重启后 SMOKE_OK；故障字典两条更新（根因+处置）。
...
- 台账 #1-#14 **全部关闭**：#11（钩子本任务文件+时长守卫，video_27/28 实证）、#11b（验收口径生效）、#12（会话指纹去重+引擎断点守卫双层，round3 实证=守卫拦截并复用）、#13（台词→SRT→烧录）、#14（§6.6 纪律挂入 dev-workflow，后续测试单驱动单会话）；
- book-16 状态=归档闭环；**book-13 同步归档**（剩余仅低优先登记保留：§3.2、P2#14）；
- 下一册 = **book-18（质量提示词固化+语音/文字清晰度加强，已批准）**。
