...
| 标题/图表后期装配 | ffmpeg **subtitles(libass)+FontName=Noto Serif CJK SC** / 数据图=SVG→overlay | ✅ 冒烟 PASS（2026-09-07）：`runs/h3/title_overlay.py` 对 video_45 叠加「第一章：重逢」→ video_48_title.mp4 中文完整清晰（drawtext+TTC 缺字形弃用，统一走 libass 与字幕链同源） |
...
审核演变与应答全文见 **`docs/history/pending-tasks-changelog.md`**（§14 模型可用性更正 / §15 一轮应答 / §16 二轮应答；后续轮次应答追加于该文件）。本文档只保留各任务**当前定稿**；任务节内“审核修订”字样仅供参考。
