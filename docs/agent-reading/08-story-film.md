...
## 配音模式(2026-09-10 用户定案:**默认 native**)
- **native(默认)**:台词原文写进该段提示词(`spoken_line_clause`:指名说话人+中文慢速清晰+口型必须对齐)
  → **H3 自己说出台词**(实测 ASR_SCORE 1.000)**并自己把台词画成画面字幕**(白字黑边、笔画正确)。
  → 因此**不再烧后期字幕**(会变成两条叠字,用户明确定为错误做法);也不再二次编码(白丢画质)。
  → 需要后期字幕时才加 `--burn-subtitle`。
- **tts(备选)**:旧行为——CosyVoice2 按角色音色配音替换原轨 + 烧字幕。
- ⚠️ **剧本 `style` 里绝不能写 `no subtitles / no on-screen text`**:那会和"要 H3 画字幕"打架,
  让字幕变弱甚至不出现。想要 H3 字幕就写正向要求(见 `config/story_radio.json` 的 style 示例)。
- H3 自绘文字清晰度的两个杠杆(实测 2026-09-10):**①提示词文字条款**(不写时 480p 常常压根不出字,
  拉了条款后同帧文字区锐度 29.2→318.9);**②分辨率**(720p 比 480p 锐 4.5 倍:318.9→1433.5,且字号更大)。
  条款已由 `h3_submit` 默认注入(`positive:text-clarity` + `negative:text`),无需 agent 每次手写。
