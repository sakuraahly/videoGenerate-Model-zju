...
成品链（S13）：**台词/配音默认=语音生成大模型 F5-TTS 本地合成（音色=官方/真人参考样本；更慢但全自然）**——正常传 tts_text 即可（不用 edge，edge=显式降级）；需要字幕/验收同链（finalize=true 为一键=本地+ASR）；参考音频/配乐底轨额外传 tts_mix_bed；要求"超分/更清晰/高清"加 upscale=true（成品 4x 超分，另耗≈5-8min）。
