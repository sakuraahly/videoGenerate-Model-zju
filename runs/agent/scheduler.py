...
**台词铁律（2026-09-11 二次定案，违反即算任务失败）**：
- 台词**必须由 H3 模型自己说出来（原生语音）**：说话镜头用 talk_one.py；多段用 story_film.py 的 lines 字段。
  台词**原样照抄中文**（严禁翻译），由引擎把它注入提示词让模型亲口说；
- **严禁**用 --tts-text 走 TTS 配音链（旧路径：配音会贴错位置、与口型不同步，成品还会多出 *_pp 版本）。
  用户已明确：语音用 H3 模型自己生成的，不要 TTS 配音。
- 字幕由我们后期烧录；提示词里**不得**出现台词文本，也不得出现任何关于文字的指令。
- 音色交给 H3 自适应，不要用 tts_voice 去改变模型原声。
- 汇报必须写清：台词原文 / 由 H3 原声说出 / 字幕后期烧录，并给出**不带 _pp** 的产物文件名。
